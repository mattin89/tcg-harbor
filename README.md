...
`scripts/sync-onepiece-data-v10.mjs` performs a build-time/server-side ingestion and writes the versioned `src/data/generated/onepiece-market-v10.json`; the browser never polls provider APIs. Prior snapshots remain preserved. The snapshot refreshed on 22 July 2026 contains 5,359 distinct card printings and 380 released sealed products (373 English, two French, and five Japanese). A user's owner-scoped collection remains separate and never expands merely because the catalog does.
...
- The Cardmarket card join is deliberately conservative: the sync scans all 21 Bandai-confirmed released English main and special booster groups through OP16, including EB01–EB03, PRB01–PRB02, and both EB04 halves inside OP14/OP15. For ambiguous V.1/V.2 rows, v10 compares each sourced artwork with every current Cardmarket candidate image before promoting an exact quote. The current snapshot has 1,280 image-verified per-art valuation mappings, including 748 alternative arts. Grouped search shows the verified regular-art price; choosing an alternate art shows that exact printing's price. Candidate low/high ranges and product-ID ordering never identify artwork or value a collection.
...
- All 5,359 card printings have an exact source-backed image. Seven formerly missing records use individually audited product/printing overrides; the sync asserts every override and never substitutes a different printing's artwork.
- All 380 currently released sealed products ship with validated, immutable local WebPs because Cardmarket blocks browser hotlinks. There are 373 exact-product images from Cardmarket, Bandai, or TCGplayer/TCGCSV. Seven outer-case rows clearly show the corresponding real contained sleeved-booster product where no trustworthy outer-case photo exists; the UI labels that relationship instead of pretending it is the case. Known placeholders are digest-blocked, and 21 announced presales/future products—including Double Pack Set Vol.12—remain release-gated.
...
- Card artwork loads from its recorded/audited remote source. Sealed-product artwork is mirrored locally as a small immutable WebP cache because Cardmarket blocks live-site hotlinks. Redistribution rights for Cardmarket and Bandai imagery still require an explicit production/legal review.
