...
export interface AppRuntimeIdentity {
  ...
  onUpdateCardPriceHistory?: (history: UserCardPriceHistory) => void | Promise<void>;
...
export default function App({ identity, guest }: AppProps = {}) {
  ...
  const productionCommunityTrading = useProductionCommunityTradingV6(Boolean(identity), identity?.userId);
  ...
  const [cardPriceHistory, setCardPriceHistory] = useState<UserCardPriceHistory>(() => {
    if (identity?.cardPriceHistory) {
      return identity.cardPriceHistory;
    }
    if (typeof window !== 'undefined' && window.localStorage) {
      return readStoredCardPriceHistory(window.localStorage);
    }
  ...
  useEffect(() => {
    if (identity?.cardPriceHistory) {
      setCardPriceHistory(identity.cardPriceHistory);
    }
  }, [identity?.cardPriceHistory]);
  ...
  useEffect(() => {
    ...
    setCardPriceHistory(updated);
    if (typeof window !== 'undefined' && window.localStorage) {
      writeStoredCardPriceHistory(window.localStorage, updated);
    }
    if (identity?.onUpdateCardPriceHistory) {
      void identity.onUpdateCardPriceHistory(updated);
    }
  }, [assets, market, identity?.onUpdateCardPriceHistory, cardPriceHistory]);
  ...
  const effectiveJoinedIds = useMemo(() => identity
    ? new Set(storeDirectory
      .filter((store) => productionCommunityTrading.isMember(store.communityId))
      .map((store) => store.id))
    : joinedIds,
  [identity, joinedIds, productionCommunityTrading.memberships, storeDirectory]);
  ...
  const [storeJoinRequests, setStoreJoinRequests] = useState<StoreJoinRequest[]>(initialStoreJoinRequests);
  const [storeRequiresMemberApproval, setStoreRequiresMemberApproval] = useState(false);
  ...
  const page = path === '/cards'
    ...
    : path === '/collection'
      ...
      : path === '/stores'
        ? <StoresPage stores={storeDirectory} joinedIds={effectiveJoinedIds} navigate={navigate} browseOnly={isGuest} onRequestAuthentication={guest?.onRequestAuthentication} />
        : path.startsWith('/stores/')
          ? isGuest && guest?.storesLoading
            ...
            : <StoreProfilePage stores={storeDirectory} storeId={path.split('/')[2]} joinedIds={effectiveJoinedIds} navigate={navigate} browseOnly={isGuest} onRequestAuthentication={guest?.onRequestAuthentication} productionTrading={identity ? productionCommunityTrading : undefined} notify={notify} />
          : path === '/communities'
            ? identity
              ? <ProductionCommunitiesPageV6 stores={storeDirectory} runtime={productionCommunityTrading} collectionAssets={assets} navigate={navigate} notify={notify}/>
                ...
                : path === '/settings'
                  ...
                  : path === '/store-admin'
                    ...
                    : path === '/scan'
                      ...
                      : path === '/join/store'
                        ? <JoinPage code={sanitizedJoinToken} joinedIds={joinedIds} setJoinedIds={setJoinedIds} navigate={navigate} notify={notify} requiresMemberApproval={storeRequiresMemberApproval} existingRequest={storeJoinRequests.find(r => r.storeId === stores.find(s => s.code.toUpperCase() === sanitizedJoinToken.toUpperCase())?.id && r.status === 'pending')} onSubmitJoinRequest={(storeId, note) => setStoreJoinRequests(prev => [...prev, { id: `req-${Date.now()}`, storeId, userId: 'user-me', userName: profileName, userInitials: profileInitials, note, requestedAt: 'Just now', status: 'pending' }])} />
                      : path.startsWith('/join/')
                        ? <JoinPage code={decodeURIComponent(path.split('/')[2] ?? '')} joinedIds={joinedIds} setJoinedIds={setJoinedIds} navigate={navigate} notify={notify} requiresMemberApproval={storeRequiresMemberApproval} existingRequest={storeJoinRequests.find(r => r.storeId === stores.find(s => s.code.toUpperCase() === decodeURIComponent(path.split('/')[2] ?? '').toUpperCase())?.id && r.status === 'pending')} onSubmitJoinRequest={(storeId, note) => setStoreJoinRequests(prev => [...prev, { id: `req-${Date.now()}`, storeId, userId: 'user-me', userName: profileName, userInitials: profileInitials, note, requestedAt: 'Just now', status: 'pending' }])} />
                        : <DashboardPage assets={assets} dailySnapshots={identity ? productionCollection.dailySnapshots : []} activity={identity ? productionActivity.activities : accountSeeds.recentActivity} activityLoading={Boolean(identity) && productionActivity.loading} collectionLoading={Boolean(identity) && productionCollection.loading} activityError={identity ? productionActivity.error : null} onRefreshActivity={productionActivity.refresh} market={market} setMarket={setMarket} period={period} setPeriod={setPeriod} kind={kind} setKind={setKind} navigate={navigate} priceHistory={cardPriceHistory} />;
...
function DashboardPage({ assets, dailySnapshots, activity, activityLoading = false, collectionLoading = false, activityError = null, onRefreshActivity, market, setMarket, period, setPeriod, kind, setKind, navigate, priceHistory }: { assets: DemoAsset[]; dailySnapshots: PortfolioDailySnapshotV2[]; activity: readonly RecentActivityItemV3[]; activityLoading?: boolean; collectionLoading?: boolean; activityError?: string | null; onRefreshActivity?: () => void | Promise<void>; market: Market; setMarket: (market: Market) => void; period: Period; setPeriod: (period: Period) => void; kind: AssetKind | 'all'; setKind: (kind: AssetKind | 'all') => void; navigate: (path: string) => void; priceHistory?: UserCardPriceHistory }) {
  ...
  const getAssetPrice = (asset: DemoAsset) => {
    const p = extractAssetAveragePrice(asset, market, priceHistory);
    return p > 0 ? p : (asset.quote[market] ?? 0);
  };
  const isPriced = (asset: DemoAsset) => getAssetPrice(asset) > 0;
  const mostValuable = (list: DemoAsset[]) => [...list].filter(isPriced).sort((a, b) => getAssetPrice(b) * b.quantity - getAssetPrice(a) * a.quantity).slice(0, 4);
  const gainers = [...filtered].filter((asset) => isPriced(asset) && asset.change[market][period] !== null && (asset.change[market][period] ?? 0) > 0).sort((a, b) => {
    const ap = a.change[market][period] ?? 0, bp = b.change[market][period] ?? 0;
    const aa = getAssetPrice(a) * a.quantity - getAssetPrice(a) * a.quantity / (1 + ap / 100);
    const ba = getAssetPrice(b) * b.quantity - getAssetPrice(b) * b.quantity / (1 + bp / 100);
  ...
  const allBySet = Object.entries(filtered.reduce<Record<string, number>>((acc, asset) => {
    ...
    if (price > 0 && Number.isFinite(price)) acc[asset.setCode] = (acc[asset.setCode] ?? 0) + price * asset.quantity;
    return acc;
  }, {})).sort((a, b) => b[1] - a[1]);
  ...
  return <div className="page dashboard-page">
    ...
    <section className="dashboard-columns"><article className="panel gainers-panel"><div className="panel-header"><div><p className="eyebrow">Momentum</p><h2>Top gainers</h2></div><Segmented value={gainRank} onChange={setGainRank} label="Gainer ranking" options={[{ value: 'percentage', label: '%' }, { value: 'absolute', label: currencyFor(market) }]} /></div><div className="gainer-list">{gainers.map((asset, index) => {
        const pct = asset.change[market][period] ?? 0, quote = getAssetPrice(asset), start = quote / (1 + pct / 100), gain = (quote - start) * asset.quantity;
...
function HoldingRank({ title, icon, assets, market, onSelect, priceHistory }: { title: string; icon: Parameters<typeof Icon>[0]['name']; assets: DemoAsset[]; market: Market; onSelect?: (asset: DemoAsset) => void; priceHistory?: UserCardPriceHistory }) {
  const getAssetPrice = (asset: DemoAsset) => {
    const p = extractAssetAveragePrice(asset, market, priceHistory);
    return p > 0 ? p : (asset.quote[market] ?? 0);
  };
  return <article className="holding-rank"><header><span><Icon name={icon}/></span><h3>{title}</h3><small>By total holding value</small></header><div>{assets.map((asset, index) => {
    const price = getAssetPrice(asset);
    const hasLivePrice = priceHistory ? extractAssetPriceFromHistory(asset, market, priceHistory) !== null : false;
    return <button key={asset.id} type="button" onClick={() => onSelect?.(asset)}><span className="rank-number">{index + 1}</span><CardArt asset={asset} size="sm"/><span className="holding-name"><strong>{asset.name}</strong><small>{asset.setCode}{asset.number ? ` · ${asset.number}` : ''}</small><em>{asset.variant}</em></span><span className="holding-qty">×{asset.quantity}</span><span className="holding-price"><strong>{formatMoney(price * asset.quantity, market)}</strong><small>{formatMoney(price, market)} each</small><em>{hasLivePrice ? 'Daily market trend' : `${marketSourceLabel(market)} · ${marketSourceDate(market)}`}</em></span></button>;
  })}</div></article>;
}
...
function StoresPage({ stores: availableStores, joinedIds, navigate, browseOnly = false, onRequestAuthentication }: { stores: Store[]; joinedIds: Set<string>; navigate: (path: string) => void; browseOnly?: boolean; onRequestAuthentication?: () => void }) {
  ...
  return <div className="page stores-page">
    ...
    <div className="mobile-map-toggle"><Segmented value={mobileMode} onChange={(mode) => { setMobileMode(mode); if (mode === 'map' && selected) setFocusNonce((nonce) => nonce + 1); }} label="Store result view" options={[{ value: 'list', label: 'List', icon: 'list' }, { value: 'map', label: 'Map', icon: 'map' }]} /></div>
    <section className="store-layout"><div className={`store-list ${mobileMode === 'map' ? 'mobile-hidden' : ''}`}><header><span><strong>{visible.length} registered stores</strong><small>Dresden · all approved locations</small></span>{identityBadgeForStores(availableStores, browseOnly)}</header>{visible.length === 0 ? <EmptyState icon="store" title="No stores found" detail="Try another store, city, postcode, or address." action={<Button variant="secondary" onClick={() => setQuery('')}>Clear search</Button>} /> : visible.map((store) => <article key={store.id} className={selected?.id === store.id ? 'selected' : ''}><button type="button" className="store-card-main" onClick={() => focusStore(store)} aria-label={`Show ${store.name} on map`}><StoreVisual store={store}/><span className="store-info"><span className="store-status"><Chip tone={browseOnly ? 'neutral' : joinedIds.has(store.id) ? 'positive' : store.communityJoinMode === 'open' ? 'gold' : 'neutral'}>{browseOnly ? 'Browse only' : joinedIds.has(store.id) ? 'Joined' : store.communityJoinMode === 'open' ? 'Open · no QR needed' : 'Visit to join'}</Chip></span><strong>{store.name}</strong><small>{store.address}</small><span className="store-open"><i />{store.hours}</span><span className="store-stats">{store.source === 'registered' ? <><em><Icon name="shield"/> Approved location</em><em><Icon name={store.communityJoinMode === 'open' ? 'users' : 'lock'}/> {store.communityJoinMode === 'open' ? 'Open test community' : 'Community stats private'}</em></> : browseOnly ? <><em><Icon name="store"/> Public store profile</em><em><Icon name="lock"/> Community stats private</em></> : <><em><Icon name="users"/> {store.members} members</em><em><Icon name="trade"/> {store.trades} active trades</em></>}</span></span></button><footer><Button variant="ghost" size="sm" onClick={() => window.open(`https://www.openstreetmap.org/?mlat=${store.latitude}&mlon=${store.longitude}#map=17/${store.latitude}/${store.longitude}`, '_blank')} icon="locate">Directions</Button><Button variant={selected?.id === store.id ? 'primary' : 'secondary'} size="sm" onClick={() => focusStore(store)} icon="map">Show on map</Button><Button variant="secondary" size="sm" onClick={() => navigate(`/stores/${store.id}`)}>View store <Icon name="chevron"/></Button></footer></article>)}</div><div ref={mapPanelRef} className={`map-panel ${mobileMode === 'list' ? 'mobile-hidden' : ''}`}><StoreMap stores={visible} selectedStoreId={selected?.id ?? null} focusNonce={focusNonce} onSelectStore={(store) => setSelected(availableStores.find((candidate) => candidate.id === store.id) ?? null)} height="100%" ariaLabel="All approved TCG Harbor stores in Dresden" active={mobileMode === 'map'} /></div></section>
...
function RegisteredStoreProfilePage({ store, navigate, joined = false, browseOnly = false, onRequestAuthentication, onJoinOpen, mutating = false }: { store: Store; navigate: (path: string) => void; joined?: boolean; browseOnly?: boolean; onRequestAuthentication?: () => void; onJoinOpen?: () => void | Promise<void>; mutating?: boolean }) {
  const openCommunity = store.communityJoinMode === 'open' && Boolean(store.communityId);
  return <div className="page store-profile-page">
    ...
    <section className={`store-hero store-${store.accent}`}>
      ...
      <div className="store-hero-copy"><div><Chip tone="positive"><Icon name="shield" size={13}/>Approved location</Chip>{openCommunity && <Chip tone="gold">Open test community</Chip>}<h2>{store.name}</h2><p>{store.address}</p></div><div className="store-hero-actions"><Button variant="secondary" onClick={() => window.open(`https://www.openstreetmap.org/?mlat=${store.latitude}&mlon=${store.longitude}#map=17/${store.latitude}/${store.longitude}`, '_blank')} icon="locate">Directions</Button>{browseOnly ? <Button type="button" onClick={onRequestAuthentication} icon="lock">Sign in to join</Button> : joined && store.communityId ? <Button onClick={() => navigate(`/communities/${store.communityId}`)} icon="users">Open community</Button> : openCommunity ? <Button disabled={mutating} onClick={() => void onJoinOpen?.()} icon="users">Join without QR</Button> : <Button onClick={() => navigate('/scan')} icon="scan">Scan at the store</Button>}</div></div>
    </section>
    <section className="store-profile-grid">
      <div className="store-profile-main">
        <article className="panel"><div className="panel-header"><div><p className="eyebrow">Verified location</p><h2>{store.communityName ?? 'Local player community'}</h2></div><span className="community-seal"><Icon name="users"/></span></div><p className="lead-copy">{browseOnly ? 'This approved location is visible to guests, but joining always requires an account.' : openCommunity ? 'This Dresden test community is deliberately open to every signed-in player, so trading features can be exercised without scanning a QR code.' : 'Visit the location and scan its current QR code to unlock its private channels, trade posts, and member directory.'}</p><div className="locked-preview"><span><Icon name={joined ? 'check' : 'lock'}/></span><div><strong>{joined ? 'Your membership is active' : 'Community details stay private'}</strong><p>{joined ? 'Open the community to create account-scoped card offers and wanted-card posts.' : 'Member counts, messages, and trades are disclosed only to active community members.'}</p></div>{joined && store.communityId && <Button size="sm" onClick={() => navigate(`/communities/${store.communityId}`)}>Open board</Button>}</div></article>
        {browseOnly ? <article className="panel guest-store-gate"><span><Icon name="lock" size={28}/></span><div><p className="eyebrow">Account required</p><h2>Sign in before joining a community</h2><p>Guest browsing never creates a membership. After signing in, {openCommunity ? 'this test community can be joined directly without a QR code.' : 'visit the store to use its physical join flow.'}</p></div><Button type="button" onClick={onRequestAuthentication}>Sign in / Create account</Button></article> : !joined && openCommunity ? <article className="panel qr-instructions"><span className="qr-placeholder"><Icon name="users" size={42}/></span><div><p className="eyebrow">Open test access</p><h2>No QR code required</h2><ol><li><span>1</span>Join with your signed-in account</li><li><span>2</span>Choose whether you offer or seek a card</li><li><span>3</span>Test money, giveaway, or card-trade terms</li></ol><p><Icon name="shield"/>Membership and posts remain protected by database policies.</p></div><div><Button disabled={mutating} onClick={() => void onJoinOpen?.()} icon="users">Join community</Button></div></article> : !joined && <article className="panel qr-instructions"><span className="qr-placeholder"><Icon name="qr" size={42}/></span><div><p className="eyebrow">Join in person</p><h2>Scan the code at the counter</h2><ol><li><span>1</span>Visit {store.name}</li><li><span>2</span>Find the current TCG Harbor QR</li><li><span>3</span>Scan and confirm the local community</li></ol><p><Icon name="shield"/>The token is revocable and never contains credentials.</p></div><div><Button variant="secondary" onClick={() => navigate('/scan')} icon="camera">Open scanner</Button></div></article>}
      </div>
...
function StoreProfilePage({ stores: availableStores, storeId, joinedIds, navigate, browseOnly = false, onRequestAuthentication, productionTrading, notify }: { stores: Store[]; storeId: string; joinedIds: Set<string>; navigate: (path: string) => void; browseOnly?: boolean; onRequestAuthentication?: () => void; productionTrading?: ProductionCommunityTradingRuntimeV6; notify: (message: string) => void }) {
  ...
  if (browseOnly || registered) return <RegisteredStoreProfilePage store={store} navigate={navigate} joined={joined} browseOnly={browseOnly} onRequestAuthentication={onRequestAuthentication} mutating={productionTrading?.mutating} onJoinOpen={productionTrading && store.communityId ? async () => {
    try {
      const outcome = await productionTrading.joinOpen(store.communityId!);
      notify(outcome === 'already_member' ? 'You are already a community member' : `Welcome to ${store.communityName ?? store.name}`);
...
function CommunityPage({ communityId, joinedIds, assets, messages, setMessages, trades, setTrades, market, navigate, notify, isStoreManager, pendingRequests, onAcceptJoinRequest, onRejectJoinRequest, profileName, profileInitials }: { communityId: string; joinedIds: Set<string>; assets: DemoAsset[]; messages: Record<string, CommunityMessage[]>; setMessages: (messages: Record<string, CommunityMessage[]>) => void; trades: TradePost[]; setTrades: (trades: TradePost[]) => void; market: Market; navigate: (path: string) => void; notify: (message: string) => void; isStoreManager?: boolean; pendingRequests?: StoreJoinRequest[]; onAcceptJoinRequest?: (id: string) => void; onRejectJoinRequest?: (id: string) => void; profileName?: string; profileInitials?: string }) {
  ...
  const sendChat = (event: FormEvent) => {
    ...
    const next: CommunityMessage = { id: `msg-${Date.now()}`, user: profileName ?? 'Mario', initials: profileInitials ?? 'MD', text: chatText.trim().slice(0, 500), time: 'Now', own: true };
    setMessages({ ...messages, [communityId]: [...chat, next] });
    setChatText('');
  };
  const deleteMessage = (messageId: string, isOwn: boolean) => {
    setMessages({ ...messages, [communityId]: chat.filter((item) => item.id !== messageId) });
    notify(isOwn ? 'Your message was deleted' : 'Message removed by store manager');
  };
  const removeMember = (user: string) => {
    setMemberList((prev) => prev.filter((m) => m.user !== user));
    notify(`${user} removed from community`);
  };
  // Insert store manager at the top of member list if isStoreManager
  const displayedMembers = isStoreManager
    ? [{ user: profileName ?? 'Store Manager', initials: profileInitials ?? 'SM', role: 'Store Manager', active: true, mutual: store.name }, ...memberList]
    : memberList;
  const pending = pendingRequests ?? [];
  return <div className="page community-page"><button className="back-link" onClick={() => navigate('/communities')}><Icon name="chevron"/>All communities</button><section className={`community-header store-${store.accent}`}><StoreVisual store={store}/><div><div><Chip tone="positive"><Icon name="shield" size={13}/>Verified member</Chip>{isStoreManager && <Chip tone="gold"><Icon name="shield" size={13}/>Store Manager</Chip>}<span>{store.city}, {store.country}</span></div><h2>{store.name}</h2><p>{store.members} local collectors · {store.trades} active trade posts</p></div><span className="online-cluster"><Avatar initials="LK" size="sm"/><Avatar initials="RB" size="sm" tone={1}/><Avatar initials="SD" size="sm" tone={2}/><small>+1 online</small></span></section><nav className="community-tabs" aria-label="Community sections">{(['overview', 'chat', 'trades', 'members'] as const).map((value) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}><Icon name={value === 'overview' ? 'dashboard' : value === 'chat' ? 'message' : value === 'trades' ? 'trade' : 'users'}/>{value[0].toUpperCase() + value.slice(1)}{value === 'chat' && <span>4</span>}{value === 'trades' && <span>{trades.filter((trade) => trade.communityId === communityId && trade.status === 'Open').length}</span>}{value === 'members' && isStoreManager && pending.length > 0 && <span>{pending.length}</span>}</button>)}</nav>
    ...
    {tab === 'chat' && <section className="chat-layout"><div className="chat-panel panel"><header><div><span className="live-pulse"/><strong>Community chat</strong><small>{store.members} members · 4 online</small></div><Button variant="ghost" size="icon" aria-label="Chat options"><Icon name="more"/></Button></header><div className="chat-date"><span>Today</span></div><div className="chat-messages">{chat.length === 0 ? <EmptyState icon="message" title="Start the conversation" detail="Be the first verified member to say hello."/> : chat.filter((message) => message.user !== blockedUser).map((message, index) => <div className={`chat-message ${message.own ? 'own' : ''}`} key={message.id}>{!message.own && <Avatar initials={message.initials} size="sm" tone={index}/>}<div><span><strong>{message.user}</strong>{message.role === 'Store Manager' && <span className="manager-badge">Store Manager</span>}<time>{message.time}</time></span><p>{message.text}</p>{message.failed && <small className="failed"><Icon name="refresh"/>Failed to send · retry</small>}<div className="message-actions">{message.own ? <button onClick={() => deleteMessage(message.id, true)}>Delete</button> : <>{isStoreManager && <button className="manager-action-btn" onClick={() => deleteMessage(message.id, false)}>Remove</button>}<button onClick={() => notify(`Message from ${message.user} reported for moderator review`)}>Report</button><button onClick={() => { setBlockedUser(message.user); notify(`${message.user} blocked`); }}>Block</button></>}</div></div></div>)}</div>{blockedUser && <div className="blocked-banner"><Icon name="lock"/>Messages from {blockedUser} are hidden.<button onClick={() => setBlockedUser('')}>Unblock</button></div>}<form className="message-composer" onSubmit={sendChat}><Avatar initials={profileInitials ?? 'MD'} size="sm"/><label><span className="sr-only">Community message</span><textarea value={chatText} onChange={(event) => setChatText(event.target.value)} placeholder="Message the community…" maxLength={500} rows={1}/><small>{chatText.length}/500</small></label><Button size="icon" aria-label="Send message" disabled={!chatText.trim()}><Icon name="send"/></Button></form><p className="realtime-note"><span className="live-pulse"/>Realtime demo channel connected · basic rate limit: 12 messages/min</p></div><aside className="chat-context panel"><h3>In this community</h3><div className="online-members">{displayedMembers.slice(0, 3).map((member, index) => <button key={member.user} onClick={() => navigate('/messages/lena')}><Avatar initials={member.initials} size="sm" tone={index}/><span><strong>{member.user}{member.role === 'Store Manager' && <span className="manager-badge">Manager</span>}</strong><small>{member.role}</small></span><i className={member.active ? 'online' : ''}/></button>)}</div><div className="chat-rules"><Icon name="shield"/><span><strong>Keep the harbor welcoming</strong><small>No sales, harassment, or private collection requests.</small></span></div></aside></section>}
    ...
    {tab === 'members' && <section className="members-section">{isStoreManager && pending.length > 0 && <div className="pending-requests-card panel"><div className="panel-header"><div><p className="eyebrow">Awaiting confirmation</p><h3>Pending join requests</h3></div></div><div className="pending-requests-list">{pending.map((req) => <div key={req.id} className="pending-request-item"><Avatar initials={req.userInitials} size="sm"/><div className="pending-request-info"><strong>{req.userName}</strong><time>{req.requestedAt}</time>{req.note && <div className="pending-request-note">"{req.note}"</div>}</div><div className="pending-request-actions"><Button size="sm" onClick={() => { onAcceptJoinRequest?.(req.id); notify(`${req.userName} accepted into community`); }}>Accept</Button><Button size="sm" variant="secondary" onClick={() => { onRejectJoinRequest?.(req.id); notify(`${req.userName} declined`); }}>Decline</Button></div></div>)}</div></div>}<div className="member-header"><div><p className="eyebrow">Verified locally</p><h2>{store.members} community members</h2><p>Only members who share this active store community can start a private conversation.</p></div><label className="search-field"><Icon name="search"/><input placeholder="Search members"/></label></div><div className="member-grid">{displayedMembers.map((member, index) => <article key={member.user} className={member.role === 'Store Manager' ? 'is-manager-card' : ''}><Avatar initials={member.initials} size="lg" tone={index}/><span><strong>{member.user}{member.role === 'Store Manager' && <span className="manager-badge">Manager</span>}</strong><small>{member.role}</small><em><i className={member.active ? 'online' : ''}/>{member.active ? 'Online now' : 'Active this week'}</em></span><div className="member-actions">{member.role !== 'Store Manager' && <Button variant="secondary" size="sm" onClick={() => navigate('/messages/lena')} icon="message">Message</Button>}{isStoreManager && member.role !== 'Store Manager' && <Button variant="secondary" size="sm" className="remove-member-btn" onClick={() => removeMember(member.user)}>Remove</Button>}</div></article>)}</div><div className="member-privacy"><Icon name="lock"/><span><strong>Membership reveals no portfolio data.</strong><small>Only cards a collector deliberately includes in a trade post are visible here.</small></span></div></section>}
...
function JoinPage({ code, joinedIds, setJoinedIds, navigate, notify, requiresMemberApproval, existingRequest, onSubmitJoinRequest }: { code: string; joinedIds: Set<string>; setJoinedIds: (ids: Set<string>) => void; navigate: (path: string) => void; notify: (message: string) => void; requiresMemberApproval?: boolean; existingRequest?: StoreJoinRequest; onSubmitJoinRequest?: (storeId: string, note: string) => void }) {
  ...
  const [requestSubmitted, setRequestSubmitted] = useState(Boolean(existingRequest));
  ...
  const submitRequest = () => {
    if (!store) return;
    onSubmitJoinRequest?.(store.id, joinNote);
    setRequestSubmitted(true);
    notify(`Request submitted to ${store.name}`);
  };
